...
foreach ($clients as $c) {
    ...
    // ── inactive_7d ──────────────────────────────
    if ($daysSinceCheckin === 7 && !wasSentToday($db, $cid, 'inactive_7d')) {
        $html = email_inactive_7d($name, $plan, $dashUrl);
        sendTriggerEmail($db, $email, "Te extrañamos, $name — ¿Todo bien? 💪", $html, $cid, 'inactive_7d', $sent, $errors);
    }

    // ── inactive_14d ─────────────────────────────
    if ($daysSinceCheckin === 14 && !wasSentToday($db, $cid, 'inactive_14d')) {
        $html = email_inactive_14d($name, $plan, $dashUrl);
        sendTriggerEmail($db, $email, "Llevamos 14 días sin saber de ti, $name", $html, $cid, 'inactive_14d', $sent, $errors);
    }

    // ── subscription_7d ──────────────────────────
    if ($daysToExpiry === 7 && !wasSentToday($db, $cid, 'subscription_7d')) {
        $html = email_renewal_reminder($name, $plan, $c['subscription_end'], $dashUrl, 7);
    ...
    // ── subscription_3d ──────────────────────────
    if ($daysToExpiry === 3 && !wasSentToday($db, $cid, 'subscription_3d')) {
        $html = email_renewal_reminder($name, $plan, $c['subscription_end'], $dashUrl, 3);
        sendTriggerEmail($db, $email, "⏰ 3 días para que venza tu plan — Actúa ahora", $html, $cid, 'subscription_3d', $sent, $errors);
    }

    // ── streak_4 ─────────────────────────────────
    if ($totalCheckins === 4 && !wasSentToday($db, $cid, 'streak_4')) {
        $html = email_streak_milestone($name, $plan, 4, $dashUrl);
        sendTriggerEmail($db, $email, "🔥 4 semanas seguidas — Eso es disciplina real, $name", $html, $cid, 'streak_4', $sent, $errors);
    }

    // ── streak_7 ─────────────────────────────────
    if ($totalCheckins === 7 && !wasSentToday($db, $cid, 'streak_7')) {
        $html = email_streak_milestone($name, $plan, 7, $dashUrl);
        sendTriggerEmail($db, $email, "🏆 7 semanas sin fallar — Eres imparable, $name", $html, $cid, 'streak_7', $sent, $errors);
    }
    ...
    if ($c['birth_date']) {
        ...
        if ($birthMMDD === $todayMMDD && !wasSentToday($db, $cid, 'birthday')) {
            $html = email_birthday($name, $plan, $dashUrl);
            sendTriggerEmail($db, $email, "¡Feliz cumpleaños, $name! 🎂 — WellCore Fitness", $html, $cid, 'birthday', $sent, $errors);
        }
    }

    // ── welcome_day1 ─────────────────────────────
    if ($daysSinceJoin === 1 && !wasSentToday($db, $cid, 'welcome_day1')) {
        $html = email_welcome_day1($name, $plan, $dashUrl);
